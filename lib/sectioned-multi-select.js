...
class SectionedMultiSelect extends PureComponent {
  ...
  _getSelectLabel = () => {
    ...
    if (renderSelectText) {
      ...
      if (typeof customSelect !== 'string') return customSelect
    }
    let label = `${selectText} (${selectedItems.length} ${selectedText})`
    if (!single && alwaysShowSelectText) label = selectText
    if (!selectedItems || selectedItems.length === 0) label = selectText
  ...
  _onChangeSearchText = (searchTerm) => {
    ...
    if (onChangeSearchText) onChangeSearchText(searchTerm)
  ...
  _closeSelector = () => {
    const { onToggleSelector } = this.props
    this.setState({ selector: false, searchTerm: '' }, () => {
  ...
  _broadcastItemObjects = (newItems) => {
    ...
    newItems.forEach((singleSelectedItem) => {
      ...
      if (item && item[displayKey]) fullItems.push(item)
    })
      ...
      <View>
        ...
        >
          <this.ModalContentWrapper>
            <this.BackdropView
              style={[
                { ...StyleSheet.absoluteFillObject, zIndex: 0, flex: 1 },
                styles.backdrop
              ]}
            />
              ...
              {!hideSearch && (
                ...
                >
                  ...
                  <TextInput
                    ...
                    cursorColor={colors.searchSelectionColor}
                    ...
                    style={[
                      { flex: 1, fontSize: 17, paddingVertical: 8 },
                      searchTextFont,
